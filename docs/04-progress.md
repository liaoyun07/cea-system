...
UI-15（2026-09-18，已部署）：00:33完成18080资源环形图现场核验，四集群读数与接口一致；56项单测、77项浏览器及桌面/窄屏通过。只更新frontend，44Flow/348执行/71应用/14数据集/13策略与其余19服务保持。内存约72%来自共享Docker宿主的节点工作集，含部分文件缓存，不是某个集群/后端独占；未取得旧系统同期指标，未作同负载内存优劣结论。无后端/API/DB/执行链变更。[验收与诊断](verification/VER-UI-015-resource-usage-charts.md)。
...
UI-12（2026-09-17，已部署）：18:18确认18080仓库自动显示全部可见镜像分页，路径改为可选筛选；中心47、edge-a 38、edge-b/edge-c各23条镜像全量比对无遗漏/重复。278项Java、54项Node、71项浏览器通过；仅前后端更新，原业务数据及其他18服务保持，不改DB/执行链。[验证](verification/VER-UI-012-registry-inventory.md)。
...
FLPAR-21（2026-09-16，已部署/验证）：线性FedAvg组合八次执行全成功，正式原MLP/候选539.69/860.84MB/s，候选最好1.041GB/s，2GB/s仍未达到；单客户端1.166→0.395秒，但平均训练重叠仅1.44，预处理/训练并集各约1.67/1.68秒。64模型192张量/120Job通过，原312执行/36Flow/19服务保持，现320执行/38Flow。未改SDK或推广业务模型；FLPAR-20追查暂缓。[记录](verification/VER-FLPAR-21-linear.md)。

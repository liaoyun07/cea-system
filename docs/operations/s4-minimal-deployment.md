...
2. 创建专用空MySQL8库及该库账号。设置README中的BACKEND_DB_URL/USER/PASSWORD、BACKEND_USER/PASSWORD/NAMESPACES。不能指向web-platform库。Flyway执行V1–V10（S5增加轮次字段，沿用停机排空要求）；从已运行的新后端升级须先排空活动执行，停止所有旧版本进程，再启动；不混跑版本、不自动repair。

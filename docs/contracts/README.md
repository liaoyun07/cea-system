...
当前为S3执行协议加S4-01资源目录；S1文档保留历史语义，S2叶子与失败语义仍适用，但顺序游标/单Worker串行已由S3替代。

- [OpenAPI 3.1](openapi.json)：19个HTTP操作，S4-01新增7个资源操作；字段以此为准。
...
没有公开Worker HTTP API，当前Worker共享数据库及应用版本。不能把本地Basic当作S4内部通信鉴权。数据库以V1–V6迁移为准；ContractTest检查路由、23个record字段映射、引用和示例，不是完整OpenAPI规范验证器。

...
- [OpenAPI 3.1](openapi.json)：47个HTTP操作（S6-01新增5个编辑/管理操作）；没有自动Flow Input派生或第二套绑定接口，字段以此为准。
...
没有公开Worker HTTP API，当前Worker共享数据库及应用版本。S4远程依赖分别使用Registry认证、Kubernetes凭据/RBAC及S3/HTTP/SQL外置凭据；终端Docker连接配置由管理员提供，不能把本地Basic当作所有内部通信鉴权。数据库以V1–V15迁移为准；ContractTest检查路由、52个record字段映射、引用和示例，不是完整OpenAPI规范验证器。

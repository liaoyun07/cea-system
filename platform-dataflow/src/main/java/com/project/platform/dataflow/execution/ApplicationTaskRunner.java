...
public final class ApplicationTaskRunner implements TaskRunner {
    public record TerminalTarget(String clusterId,String dockerContext) {}
    public record Prepared(String clusterId,ContainerTask.Spec spec,Map<String,String> inputUris,Map<String,String> inlineFiles,String dockerContext) {}
    ...
    public ApplicationTaskRunner(ApplicationCatalogService applications,ResourceCatalogService resources,ImageDistributionService images,
            ...
            Function<WorkerJob,TerminalTarget> terminals,JsonCodec json,BindingResolver bindings) {
        ...
        this.storage=storage;this.identities=identities;this.terminals=terminals;this.json=json;this.bindings=bindings;
    }
    @Override public WorkerJob.Result run(TaskContext context) throws Exception {
        ...
        boolean clusterTask=job.task().container().execution()==com.project.platform.runtime.model.FlowDefinition.ContainerExecution.CLUSTER;
        ...
        try {
            ...
            if(saved==null && context.cancellation()!=null) {
                if(clusterTask)placement.release(namespace,key);return WorkerJob.Result.failed(context.cancellation());
            }
            Prepared plan=saved==null?prepare(context,namespace,key):json.read(saved,Prepared.class);
            if(plan==null){if(clusterTask)placement.release(namespace,key);return WorkerJob.Result.failed("attempt cancelled before dispatch");}
            ...
            if(plan.dockerContext()!=null)return docker.run(context,plan.dockerContext(),plan.spec(),files);
            ...
            if(context.prepared()==null){if(clusterTask)placement.release(namespace,key);return WorkerJob.Result.failed(ex.getMessage());}
    ...
    private Prepared prepare(TaskContext context,String namespace,String key) throws Exception {
        ...
        TerminalTarget terminal=c.execution()==com.project.platform.runtime.model.FlowDefinition.ContainerExecution.TERMINAL?terminals.apply(context.job()):null;
        String cluster;
        if(terminal!=null)cluster=terminal.clusterId();
        else {
        ...
        var plan=new Prepared(cluster,new ContainerTask.Spec(image.image(),c.command(),env,List.copyOf(files),c.outputFiles()),inputUris,inlineFiles,terminal==null?null:terminal.dockerContext());

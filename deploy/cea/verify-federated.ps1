...
foreach ($taskRun in $taskLeaves) {
    ...
    if ($taskUri.Scheme -ne 's3' -or $taskUri.Host -ne 'cea-artifacts') { throw 'Unexpected artifact URI' }
    $taskCopies += @{source="local/$($taskUri.Host)$($taskUri.AbsolutePath)";destination=$taskFile}
...
foreach ($taskCopy in $taskCopies) {
    if ($taskCopy.source -notmatch '^local/cea-artifacts/[A-Za-z0-9/._-]+$') { throw 'Unexpected artifact key characters' }
...
Write-Output "PASS: $Algorithm, actual four clusters, 11 Jobs, two rounds, independent numerical audit. Evidence: $taskEvidence"

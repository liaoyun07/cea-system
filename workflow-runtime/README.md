...
S5-04a增加DockerTaskRunner，与KubernetesJobRunner共享ContainerTask命令/文件协议，仍由原Worker租约链消费，不另建状态机。部署连接及终端归属由server装配，runtime无业务模块依赖，见[终端执行协议](../docs/contracts/s5-terminal-docker.md)。

S4增加真实KubernetesJobRunner、租约TaskContext及HTTP/只读SQL任务；应用/资源适配由dataflow通过现有TaskRunner接口装配，runtime不反向依赖业务。详见[S4 Job协议](../docs/contracts/s4-job-execution.md)和[通用任务](../docs/contracts/s4-common-tasks.md)。

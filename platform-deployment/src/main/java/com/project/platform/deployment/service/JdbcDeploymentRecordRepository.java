...
public final class JdbcDeploymentRecordRepository {
    public record Observation(String id,String namespace,String clusterId,String name,String uid,Long generation,int replicas,
    ...
    public String begin(String namespace,String cluster,String name,String application,String version,String operation,int replicas,Instant now,Instant deadline) {
        String id=UUID.randomUUID().toString();
        jdbc.update("INSERT INTO dep_deployment_record(id,namespace,cluster_id,deployment_name,application_id,version,operation,target_replicas,state,started_at,last_observed_at,deadline_at,timing_valid) VALUES(?,?,?,?,?,?,?,?,'PREPARING',?,?,?,?)",
                id,namespace,cluster,name,application,version,operation,replicas,Timestamp.from(now),Timestamp.from(now),Timestamp.from(deadline),replicas>0);
        return id;
    }
    ...
    public List<Observation> active() {
        return jdbc.query("SELECT * FROM dep_deployment_record WHERE state IN ('PREPARING','OBSERVING') ORDER BY last_observed_at LIMIT 100",
                (rs,row)->new Observation(rs.getString("id"),rs.getString("namespace"),rs.getString("cluster_id"),rs.getString("deployment_name"),rs.getString("target_uid"),
    ...
    public List<DeploymentRecord> list(String namespace,String cluster,String name,int limit,int offset) {
        return jdbc.query("SELECT * FROM dep_deployment_record WHERE namespace=? AND cluster_id=? AND deployment_name=? ORDER BY started_at DESC,id DESC LIMIT ? OFFSET ?",
                (rs,row)->record(rs),namespace,cluster,name,limit,offset);
    }

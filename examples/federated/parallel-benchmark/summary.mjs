...
const folder = resolve(import.meta.dirname, `../../../.local/cea/${replicated ? 'par02' : 'par01'}`);
...
for (const current of cases) {
  ...
  if (replicated) for (const result of success) {
    ...
    assert.equal(training.length, current.clients * 2);
    assert.equal(new Set(training.map(report => `${report.round}/${report.item}`)).size, current.clients * 2);
...
for (const row of summaries.filter(r => replicated ? r.clients > 3 : r.clients === 6 && r.concurrency === 6)) {

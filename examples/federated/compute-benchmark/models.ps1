...
$taskEvidence = Join-Path $taskRepository '.local/cea/par24'
$taskAudit = Get-Content (Join-Path $taskEvidence 'audit.json') -Raw | ConvertFrom-Json
if ($taskAudit.result -ne 'PASS' -or $taskAudit.copies.Count -lt 1) { throw 'Job audit required' }
$taskScript = @'
...
foreach ($taskCopy in $taskAudit.copies) {
    ...
    if ($taskCopy.destination -notmatch '^par24/models/[0-3]/(init-0|train-[1-3]|aggregate-1)\.pt$') { throw 'Unexpected destination' }
